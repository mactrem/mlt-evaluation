...
function decodeLongColumn(data: Uint8Array, offset: IntWrapper, column: Column,
    ...
    const dataStreamMetadata = StreamMetadataDecoder.decode(data, offset);
    const vectorType = IntegerStreamDecoder.getVectorType(dataStreamMetadata,
    ...
    if (vectorType === VectorType.FLAT) {
        ...
        const id = IntegerStreamDecoder.decodeSequenceLongStream(data, offset,
            dataStreamMetadata);
        return new LongSequenceVector(column.name, id[0], id[1],
...
function decodeIntColumn(data: Uint8Array, offset: IntWrapper, column: Column, scalarColumn: ScalarColumn,
    ...
    if (vectorType === VectorType.FLAT) {
        ...
        const id = IntegerStreamDecoder.decodeSequenceIntStream(data, offset,
            dataStreamMetadata);
        (dataStreamMetadata as RleEncodedStreamMetadata).numValues
        return new IntSequenceVector(column.name, id[0], id[1],
            (dataStreamMetadata as RleEncodedStreamMetadata).numRleValues);
    } else {

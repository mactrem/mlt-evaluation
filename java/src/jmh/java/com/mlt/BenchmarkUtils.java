...
public class BenchmarkUtils {
  ...
  public static void encodeTile(
          ...
          List<byte[]> encodedMvtTiles,
          List<ByteArrayInputStream> encodedMvtTiles2,
          List<byte[]> compressedMvtTiles,
          List<byte[]> encodedMltTiles,
          List<ColumnMapping> columnMappings)
          throws IOException {
    encodedMvtTiles.add(encodedMvtTile);
    encodedMvtTiles2.add(new ByteArrayInputStream(encodedMvtTile));
    compressedMvtTiles.add(EncodingUtils.gzip(encodedMvtTile));

    var optimization =
            new FeatureTableOptimizations(false, false, Optional.of(columnMappings));
    var optimizations =
            TestSettings.OPTIMIZED_MVT_LAYERS.stream()
                    .collect(Collectors.toMap(l -> l, l -> optimization));
    var encodedMltTile =
            MltConverter.convertMvt(mvt,
                    new ConversionConfig(true, true, optimizations), metadata);
    encodedMltTiles.add(encodedMltTile);
  }

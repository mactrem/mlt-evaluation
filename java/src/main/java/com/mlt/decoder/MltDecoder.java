...
public class MltDecoder {
  ...
  public static FeatureTable[] decodeMlTileVectorized(
          byte[] tile, MltTilesetMetadata.TileSetMetadata tileMetadata, Map<String, Field> nestedScheme) {
    var offset = new IntWrapper(0);
    var featureTables = new FeatureTable[tileMetadata.getFeatureTablesCount()];
    while (offset.get() < tile.length) {
      var version = tile[offset.get()];
      ...
      GeometryVector geometryVector = null;
      var propertyVectors = new ArrayList<Vector>();
      //TODO: refactor to use also for nested data the vector concept -> only quick and dirty evaluation
      Map<String, List<? extends JsonNode>> nestedPropertyColumns = new HashMap<>();
      var columList = metadata.getColumnsList();
      for (var columnMetadata : columList) {
        var columnName = columnMetadata.getName();
        var numStreams = DecodingUtils.decodeVarint(tile, offset, 1)[0];
      ...
      /* Currently the nested properties have to be placed after the scalar properties in the file */
      var nestedColumnsMetadata = nestedScheme.entrySet().stream().
              filter(e -> e.getKey().split("\\.")[0].equals(metadata.getName())).
              map(Map.Entry::getValue).toList();
      for(var field : nestedColumnsMetadata) {
        var columnName = field.name;
        var numStreams = DecodingUtils.decodeVarint(tile, offset, 1)[0];
        var jsonNode = ComplexColumnDecoder.decodeComplexColumn(tile, offset, field, null,
                columnName, null);
        nestedPropertyColumns.put(columnName, jsonNode);
      }

      var featureTable =
              new FeatureTable(
                      metadata.getName(), idVector, geometryVector, propertyVectors.toArray(new Vector[0]),
                      nestedPropertyColumns);
      featureTables[featureTableId] = featureTable;
    }

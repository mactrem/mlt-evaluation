...
public class VectorizedDecodingUtils {
  ...
  public static ByteBuffer decodeNullableBooleanRle(
    ...
    for (var i = 0; i < nullabilityBuffer.size(); i++) {
      nullableBitset.set(i, nullabilityBuffer.get(i) && bitVector.get(valueCounter++));
    }

    //TODO: handle the case when all values are false as toByteArray return empty array
    //Introduce ConstBooleanVector for that case
  ...
  public static VectorType getVectorType(StreamMetadata streamMetadata, int numFeatures) {
    ...
    if (logicalLevelTechnique1.equals(LogicalLevelTechnique.DELTA)
        ...
        && (((RleEncodedStreamMetadata) streamMetadata).runs() == 1 ||
            ...
            && streamMetadata.numValues() == numFeatures
    ) {
  ...
  public static VectorType getVectorTypeBooleanStream(
      int numValues, int byteLength, byte[] data, IntWrapper offset) {
    var valuesPerRun = 131;
    // TODO: use VectorType metadata field for to test which VectorType is used
    return (Math.ceil((double) numValues / valuesPerRun) * 2 == byteLength)
            &&
            /* Test the first value byte if all bits are set to true in the range or all to false  */
            ((data[offset.get() + 1] & 0xFF) == ((Integer.bitCount(numValues) << 2) - 1) ||
                    (data[offset.get() + 1] & 0xFF) == 0)
        ? VectorType.CONST
        : VectorType.FLAT;
  }
}

...
public class MltConverter {
  ...
  public static Pair<MltTilesetMetadata.TileSetMetadata, Map<String, Field>> createTilesetMetadataFromNestedJsonProperties(
    ...
    var nestedColumnsScheme = new HashMap<String, Field>();
      ...
      for (var layer : tile.layers()) {
        ...
        for (var feature : layer.features()) {
          ...
          for (var property : properties) {
            ...
            var jsonNode = parseJson(property.getValue().toString());
            if (scalarType.equals(MltTilesetMetadata.ScalarType.STRING) && jsonNode != null) {
              var nestedFieldId = layer.name() + "." + mvtPropertyName;
              var nestedColumn = nestedColumnsScheme.get(nestedFieldId);
              nestedColumn = createNestedSchema(jsonNode, nestedColumn);
              nestedColumn.name = mvtPropertyName;
              nestedColumnsScheme.put(nestedFieldId, nestedColumn);
              continue;
            }
    ...
    var tilest = tilesetBuilder.build();
    return Pair.of(tilest, nestedColumnsScheme);
  }
  ...
  public static byte[] convertMvt(
    ...
    for (var mvtLayer : mvt.layers()) {
      ...
      var nestedPropertyColumnMetadata = nestedPropertyScheme.entrySet().stream().
              filter(e -> e.getKey().split("\\.")[0].equals(featureTableName)).
      ...
      var encodedPropertyColumns =
              encodeComplexPropertyColumns(
                      config, featureTableMetadata, nestedPropertyColumnMetadata, sortedFeatures,
                      featureTableOptimizations);
  ...
  private static Field createNestedSchema(JsonNode jsonNode, Field field) {
    /*
     * -> ComplexColumn -> ComplexType -> struct and list
     * -> Internal Node
     *   -> length stream
     *     -> if array -> add entry to length stream
     *     -> if object -> no entry
     *   -> present stream
     *     -> add entry to present stream
     * -> leaf
     * */
    ...
    if(jsonNode.isObject()){
      ...
      final var structField = (StructField)field;
      jsonNode.fieldNames().forEachRemaining(fieldName -> {
        var childNode = jsonNode.get(fieldName);
        if(!childNode.isNull()){
          var childField = createNestedSchema(childNode, structField.childs.get(fieldName));
          structField.childs.put(fieldName, childField);
        }
      });
      ...
      if(field == null){
        field = new RepeatedField();
        field.nullable = true;
      }

      var repeatedField = (RepeatedField)field;
      for (var childNode : jsonNode) {
        repeatedField.childField = createNestedSchema(childNode, repeatedField.childField);
      }
      ...
      switch (jsonNode.getNodeType()) {
        case JsonNodeType.BOOLEAN:
          scalarField.type = MltTilesetMetadata.ScalarType.BOOLEAN;
          return scalarField;
        case JsonNodeType.NUMBER:
          //TODO: also handle unsigned integers
          if(jsonNode.isInt()){
            scalarField.type = MltTilesetMetadata.ScalarType.INT_32;
            return scalarField;
          }
          else if(jsonNode.isLong()){
            scalarField.type = MltTilesetMetadata.ScalarType.INT_64;
            return scalarField;
          }
          else if(jsonNode.isFloat()){
            scalarField.type = MltTilesetMetadata.ScalarType.FLOAT;
            return scalarField;
          }
          else if(jsonNode.isDouble()){
            scalarField.type = MltTilesetMetadata.ScalarType.DOUBLE;
            return scalarField;
          }
          break;
        case JsonNodeType.STRING:
          scalarField.type = MltTilesetMetadata.ScalarType.STRING;
          return scalarField;
      }
    }

    throw new RuntimeException("Scalar type not supported");
  }
  ...
  private static byte[] encodeComplexPropertyColumns(
          ConversionConfig config,
          MltTilesetMetadata.FeatureTableSchema featureTableMetadata,
          List<Field> nestedPropertyMetadata,
          List<Feature> sortedFeatures,
          FeatureTableOptimizations featureTableOptimizations)
          throws IOException {
    var propertyColumns = filterPropertyColumns(featureTableMetadata);
    return PropertyEncoder.encodeComplexPropertyColumns(
            propertyColumns,
